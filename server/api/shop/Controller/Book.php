...
class Book extends Controller
{
    ...
    public function putBook($params  = false){
        ...
        if($countRecGanre!=0){
            ...
            foreach ($ganres as $ganre) {
                if((int)$ganre>0){
                    $model->insertGanreByBookId(['book_id'=>$params->id, 'ganre_id'=>$ganre]);
                    $kolGan++;
                }
            }
        }
        ...
        if($countRecAuthor!=0){
            ...
            foreach ($authors as $author) {
                if((int)$author>0){
                    $model->insertAuthorByBookId(['book_id'=>$params->id, 'author_id'=>$author]);
                    $kolAuthr++;
                }
            }
        }
        
        return ['status'=>200, 'data'=>['bookEdit'=>1,'ganreUpd'=>$kolGan,'authorUpd'=>$kolAuthr]];
    }
